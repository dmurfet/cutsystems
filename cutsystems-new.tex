...
The paper is structured as follows: in Section \ref{section:background} we recall the definition of superbicategories and Clifford algebras, and introduce the Clifford thickening of a supercategory. In Section \ref{section:lg_cut_system} we define $\cat{L}$ and in Section \ref{section:equivllg} we prove it is equivalent to $\LG$ as a superbicategory without units. We do not consider unit $1$-morphisms here because $\cat{L}$ does not make working with units any simpler.
...
$$
  ...
  \put(0,0){\scalebox{0.6}{\includegraphics{pic02.eps}}}
  \put(0,0){
...
\begin{theorem} There is an equivalence $\cat{Z}: \LG \lto \cat{L}$ of superbicategories without units.
\end{theorem}
\begin{proof}
...
\begin{remark} Obviously the $1$-morphisms $\Delta_W: W \lto W$ which are the units in $\LG$ are, by virtue of the equivalence $\mathcal{Z}$, also units in $\cat{L}$, so that this is an honest bicategory and $\mathcal{Z}$ an equivalence of bicategories. The reason we do not begin with units in $\cat{L}$ is that the unitors are no simpler in $\cat{L}$ than $\LG$, so there is no real point to introducing them.
\end{remark}
