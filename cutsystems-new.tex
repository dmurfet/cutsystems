...
\begin{abstract}
The bicategory $\LG$ of Landau-Ginzburg models has polynomials as objects and matrix factorisations as $1$-morphisms. The composition of $1$-morphisms naturally produces infinite rank matrix factorisations, which is a nuisance. In this paper we define a bicategory $\L$ which is equivalent to $\LG$ but for which composition of $1$-morphisms produces finite rank matrix factorisations equipped with the action of a Clifford algebra.
\end{abstract}
...
The bicategory $\LG$ of Landau-Ginzburg models has a rich structure, as can be seen by its important role in topological field theory \cite{McNameethesis,lgdual,genorb} homological knot invariants \cite{kr0401268} and $2$-representations of quantum groups \cite{??}, but the definition is still relatively elementary. This makes $\LG$ an attractive algebraic object of study, but for one major wrinkle: the composition operation for $1$-morphisms is infinitary.

The objects of $\LG$ are potentials $W(x)$, while $1$-morphisms $W(x) \lto V(y)$ are matrix factorisations of the difference $V(y) - W(x)$ over $\nC[x,y]$. Such a matrix factorisation is a $\nZ_2$-graded free $\nC[x,y]$-module $X = X^0 \oplus X^1$ together with an odd operator $d_X$ satisfying $d_X^2 = (V(y) - W(x)) \cdot 1_X$. For a third potential $U(z)$, the composition of $1$-morphisms is a functor
\begin{gather*}
...
\ee
This is the analogue of convolution of Fourier-Mukai kernels in the matrix factorisation setting and suffers from the same defect, namely, the resulting matrix factorisation $Y \circ X$ is a free module of \emph{infinite rank} over $\nC[x,z]$ which is only homotopy equivalent (that is, isomorphic in the homotopy category) to a finite rank matrix factorisation. While this finite model can be proven abstractly to exist, there is no obvious way to describe it as a function of the input data $Y,X$ viewed as matrices of polynomials. This need to deal throughout with infinite rank matrix factorisations introduces a certain minimal level of complexity into the theory, which for many applications is undesirable.

Together with Dyckerhoff we initiated in \cite{dm1102.2957} the study of these finite models in terms of homological perturbation and Atiyah classes, and in the present paper this program reaches its logical conclusion. We introduce a bicategory $\L$ in which the objects are again potentials and where the $1$-morphisms are finite rank matrix factorisations equipped with the structure of a representation of a Clifford algebra. We will prove that this bicategory is equivalent to $\LG$, but is much simpler - in particular, it has a composition operation which is described by a finite number of polynomial functions of the input data.

More precisely: the composition of $1$-morphisms in $\L$ is a functor
\begin{gather}
\L(V,U) \times \L(W,V) \lto \L(W,U)\,,\\
(Y,X) \mapsto Y \l X
\end{gather}
which we call the \emph{cut operation}. The definition of the cut is very simple. With $J_V = \nC[y]/(\partial V)$ the Jacobi algebra of $\nC$ (which is a finite-dimensional $\nC$-vector space by the assumption that $V(y)$ is a potential) the cut of $Y$ and $X$ is a finite rank matrix factorisation
\be
Y \l X = Y \otimes_{\nC[y]} J_V \otimes_{\nC[y]} X = ( Y \otimes_{\nC[y]} X ) / (\partial V) (Y \otimes_{\nC[y]} X )
\ee
over $\nC[x,z]$

equipped with odd operators $\ferm_i, \fermc_i$ satisfying Clifford relations
\begin{gather*}
\ferm_i e = \ferm_i = e \ferm_i\,, \\
\fermc_i e = \fermc_i = e \fermc_i\,.
\end{gather*}
up to homotopy (see Theorem \ref{theorem:cut_is_rep}). The differential on $Y \l X$ is induced from the differential on $Y \otimes X$ and can be written explicitly as a matrix over $\nC[x,z]$ in terms of the polynomial entries of $d_Y, d_X$. The Clifford action and the matrices which define the Clifford actions on $Y,X$. In particular, infinite rank matrix factorisations are never involved. In the remainder of the introduction we sketch how the cut operation arises from homological perturbation, why Clifford algebras are involved, where the equivalence $\L \cong \LG$ comes from, and our motivations for pursuing this ``finite model'' $\L$ of $\LG$. %In this sense the bicategory $\L$, which we prove is equivalent to $\LG$, provides a ``finite model'' of $\LG$. This has various applications, which we will describe later in the introduction.

The main theorem (Theorem \ref{theorem:htpy_equivalence_main}) says
\begin{equation}\label{eq:final_finite_model}
\xymatrix@C+4pc{
Y \l X \ar@<-1ex>[r]_-{\Phi^{-1}} & S_m \otimes_k ( Y \otimes X )\ar@<-1ex>[l]_-{\Phi}
}
\end{equation}
The paper is structured as follows: in Section \ref{section:background} we recall the definition of superbicategories and Clifford algebras, and introduce the Clifford thickening of a supercategory. In Section \ref{section:lg_cut_system} we define $\L$ and in Section \ref{section:equivllg} we prove it is equivalent to $\LG$ as a superbicategory without units. We do not consider unit $1$-morphisms here because $\L$ does not make working with units any simpler.

\subsection{$A_\infty$-algebras and minimal models}

The relationship between $\LG$ and $\L$ is similar to the relationship between a DG-algebra and its $A_\infty$-minimal model. Given a DG algebra $A$ over a field $k$, which is infinite-dimensional but has finite-dimensional cohomology, the $A_\infty$-minimal model is an $A_\infty$-structure on the cohomology $H^*(A)$ which is $A_\infty$-quasi-isomorphic to $A$. The tradeoff being made is that $A$ is infinite-dimensional but with a simple structure (i.e. one multiplication map $m_2$) while $H^*(A)$ is finite-dimensional but with possibly an infinite family of higher multiplications. In $\LG$ the composition operation is not described by a finite amount of data, and the price that one pays by going to $\L$ where this is the case, is that the composition has additional structure (a Clifford action). 
There is a more precise link to $A_\infty$-minimal models. A special case of the equivalence between $\L$ and $\LG$ gives, for a matrix factorisation $X$ of the potential $W(x_1,\ldots,x_m)$ viewed, a strong deformation retract
\be
\Phi: S_m \otimes_k \Hom_R(X,X) \lto X^{\vee} \l X\,.
\ee
where $S_m = \bigwedge( k \theta_1 \oplus \cdots \oplus k \theta_m )$ is an exterior algbera, $\Hom_R(X,X)$ is the Hom complex. This deformation retract can be used as the input data for the usual construction of an $A_\infty$-minimal on the left hand side, viewed as a DG-algebra.

\subsection{A geometric enrichment of $\LG$}
...
\subsection{Semantics of linear logic}

In forthcoming work we will use the results of this paper to define a novel semantics of linear logic, in which formulas are interpreted by tuples $(\md{X}, \nC[x], W)$ where $\md{X}$ is a scheme and $W \in \nC[x]$ is a potential. Proofs in the logic are interpreted by certain correspondences between these pairs. To explain briefly why we are forced to use $\L$ rather than $\LG$, we will use the denotation of the Church numeral $\underline{2}$ (see \cite{murfet_ll}).
...
\[
\ferm_i \mapsto \theta_i \wedge (-), \qquad \fermc_i \mapsto \theta_i^*\, \lrcorner\, (-)
\]
...
\[
e_n = \fermc_1 \cdots \fermc_n \ferm_n \cdots \ferm_1\,.
\]
...
\begin{align*}
\cat{T}^{\star}( (X,n), (Y,m) ) &= \Big\{ \alpha \in \cat{T}(X,Y) \l \alpha \ferm_i = 0 \text{ for } 1 \le i \le n \text{ and }\\
&\qquad \qquad\qquad \qquad \;\; \fermc_i \alpha = 0 \text{ for } 1 \le i \le m \Big\}\,.
\end{align*}
...
\begin{align*}
\alpha e_n &= \alpha \fermc_1 \cdots \fermc_n \ferm_n \cdots \ferm_1\\
&= \alpha \left( \fermc_1 \cdots \fermc_{n-1} \ferm_{n-1} \cdots \ferm_1 - \fermc_1 \cdots \ferm_n \fermc_n \cdots \ferm_1 \right)\\
&= \alpha \fermc_1 \cdots \fermc_{n-1} \ferm_{n-1} \cdots \ferm_1
\end{align*}
since in the second summand within the brackets, there is nothing to stop $\ferm_n$ from commuting to the left and annihilating with $\alpha$. Proceeding inductively, we see that $\alpha e_n = \alpha$. The proof that $e_n \alpha = \alpha$ is similar.
...
\[
f_0 \ferm_i = f_0 e_n \ferm_i = f_0 \fermc_1 \cdots \fermc_n \ferm_n \cdots \ferm_1 \ferm_i = 0\,.
\]
...
Thus a $1$-morphism $W \lto V$ in $\L$ is a finite rank matrix factorisation $X$ of $V(y) - W(x)$ together with an idempotent endomorphism $e$ and a family of odd operators $\ferm_i, \fermc_i$ satisfying Clifford relations and the equations (all up to homotopy)
\begin{gather*}
\ferm_i e = \ferm_i = e \ferm_i\,, \\
\fermc_i e = \fermc_i = e \fermc_i\,.
\end{gather*}
Most of the time we only care about $1$-morphisms where $e = 1$, and let us only describe $2$-morphisms in this case. For matrix factorisations $X,Y$ of $V(y) - W(x)$ with respective Clifford actions $\{ \ferm_i, \fermc_i \}_{i=1}^a$ and $\{ \rho_j, \rho_j^\dagger \}_{j=1}^b$ the $2$-morphisms $\phi: (X,a) \lto (Y,b)$ in $\L$ are by Lemma \ref{lemma:equivcliffordthick} in bijection with morphisms of matrix factorisations $\phi: X \lto Y$ satisfying
\[
\phi \ferm_i = 0\,, \qquad \rho_j^\dagger \phi = 0\,, \qquad 1 \le i \le a, \quad 1 \le j \le b\,.
\]
...
\fermc_i = \At_i\,, \qquad \ferm_i = - \partial_{y_i}(d_X) - \frac{1}{2} \sum_q \partial_{y_q y_i}(V) \At_{q}\,.
\end{equation}
\end{definition}
...
&= [d, -[\partial_{t_j}, \At_i]]\,.
\end{align*}
Hence $[\fermc_i, \fermc_j] = [d, h_{ij}]$ where $h_{ij} = - [ \partial_{t_j}, \At_i ]$. Next we compute
\begin{align*}
...
\begin{align*}
[ \ferm_i, \fermc_j] &= \big[- \partial_{y_i}(d_X) - \frac{1}{2} \sum_q \partial_{y_q y_i}(V) \At_{q}, \At_j \big]\\
...
\begin{align*}
[ \ferm_i, \ferm_j ] &= \Big[ \partial_{y_i}(d_X) + \frac{1}{2} \sum_q \partial_{y_q y_i}(V) \At_{q}\; , \; \partial_{y_j}(d_X) + \frac{1}{2} \sum_q \partial_{y_q y_j}(V) \At_{q} \Big]\\
...
\]
relative to the ring morphism $k[q,x] \lto k[q,x] [ \partial_{y_1} U, \ldots, \partial_{y_p} U ]$. Specifically, the generator $\fermc_i$ of $C_p$ is the commutator $[d_{M}, \partial_s]$ where $s = \partial_{y_i} U$. But this operator can be identified with an Atiyah class of the linear factorisation
...
\[
e_m' = \ferm_1 \cdots \ferm_m \fermc_m \cdots \fermc_1\,.
\]
Observe that each of the $\ferm_i$'s acts as $- \lambda_i$ because all of the $\fermc_j$'s to the right. Hence
\begin{align*}
e_m' &= (-1)^{\binom{m}{2}} \ferm_1 \cdots \ferm_m \fermc_1 \cdots \fermc_m \\
...
\[
\Phi \theta_i^* \Phi^{-1} \simeq \pi \cat{T}(\theta_i^*) \sigma_\infty = \pi \theta_i^* \sigma_\infty \simeq \At_i = \fermc_i\,.
\]
...
\[
e_m = \fermc_1 \cdots \fermc_m \ferm_m \cdots \ferm_1 \in C_m
\]
...
Using the bicategory $\L$ we can study the complex $\Hom_R(Y,X)$ for two finite rank matrix factorisations $X,Y$ of a potential $V \in R = k[y_1,\ldots,y_m]$. In particular, we will explain how the homotopy relation on morphisms of matrix factorisations is effectively computable.
...
\end{align*}
The matrix of the Atiyah class as an operator $\fermc$ on $Y^{\vee} \l X$ is therefore
\begin{equation}\label{eq:aityah_class_hom_example}
\fermc = \begin{pmatrix} 0 & 0 & -[R_{N-i}] & -[R_{j}] \\ 
0 & 0 & -[R_{N-j}] & -[R_{i}] \\
[R_i] & -[R_j] & 0 & 0 \\
...
By definition
\[
\ferm = - \partial_y(d_X) - \frac{1}{2} N (N-1) y^{N-2} \fermc
\]
...
\[
\Hom_{C_n}^0( S_n, X ) \cong \big\{ x \in X^0 \l \fermc_i \cdot x = 0 \text{ for all } 1 \le i \le n \big\}
\]
defined by $f \mapsto f(1)$. If $X$ is a right $C_n$-module there is a $k$-linear isomorphism
\[
\Hom^0_{C_n}( (S_n)^*, X ) \cong \big\{ x \in X^0 \l x \cdot \ferm_i = 0 \text{ for all } 1 \le i \le n \big\}
\]
...
\[
\cat{C}_{C_n}( S_n \otimes_k X, Y ) \cong \{ \delta \in \cat{C}(X,Y) \l \fermc_i \circ \delta = 0 \text{ for all } 1 \le i \le n \}
\]
...
\begin{align*}
\cat{C}_{C_q}( S_{q,p} \otimes_{C_p} X, Y ) \cong \{ \delta: X \lto Y \l \delta \circ \ferm_i &= 0 \text{ for } 1 \le i \le p \text{ and }\\ \fermc_i \circ \delta &= 0 \text{ for } 1 \le i \le q \}
\end{align*}
...
&\cong \{ \delta \in \Hom^0_{C_q}( S_p^*, \cat{C}^*(X,Y) ) \l \fermc_i \circ \delta = 0 \text{ for } 1 \le i \le q \}\\
&\cong \{ \delta: X \lto Y \l \delta \circ \ferm_i = 0 \text{ for } 1 \le i \le p \text{ and } \fermc_i \circ \delta = 0 \text{ for } 1 \le i \le q \}
\end{align*}
as claimed.
\end{proof}
